...
#plt.xlim(x_range)
#plt.ylim(y_range)
#plt.savefig(savef+'Fig6a_left_v3.6.pdf')
...
plt.xlim(x_range)
plt.ylim(y_range)
#plt.savefig(savef+'Fig6a_right.pdf')

#%% Fig 6b
ff = '/media/nel/storage/fiola/R2_20190219/result/'
r = np.load(ff+'Fig6c_result_v3.6.npy', allow_pickle=True).item()
p = np.load(ff+'Fig6c_prediction_v3.6.npy', allow_pickle=True).item()
plt.figure()
...
#plt.savefig(savef + 'Fig6_supp_raster_plot_v3.6.pdf')  
...
tp_gt, tp_comp, fn_gt, fp_comp, performance_suite2p = compare_components(s2p_estimate, cnm_estimate,
                                                                         ...
                                                                         plot_results=True)
#plt.savefig(savef + 'Fig6_supp_spatial_comparison1_v3.7.pdf')
...
tp_gt, tp_comp, fn_gt, fp_comp, performance_suite2p = compare_components(s2p_estimate, caiman_estimate,
                                                                         ...
                                                                         plot_results=True)
#plt.savefig(savef + 'Fig6_supp_spatial_comparison2_v3.7.pdf')  #1441/1789
print(performance_suite2p)
...
tp_gt, tp_comp, fn_gt, fp_comp, performance_suite2p = compare_components(s2p_estimate, onacid_estimate,
                                                                         ...
                                                                         plot_results=True)
plt.savefig(savef + 'Fig6_supp_spatial_comparison3_v3.10.pdf')  
...
#                                                                          plot_results=True)
# plt.savefig(savef + 'Fig6_supp_spatial_comparison4_v3.8.pdf')  
print(len(tp_gt) / (len(tp_gt) + len(fn_gt)))
...
fig.tight_layout()
plt.show()
plt.savefig(savef + 'Fig6_supp_detection_F1_v3.10.pdf') 
...
plt.tight_layout()
plt.savefig(savef + 'Fig6_supp_fiola_processing_v3.7.pdf')  
...
plt.tight_layout()
plt.savefig(savef + 'Fig6_supp_deconvolution_v3.7.pdf')  
...
for ii, lag in enumerate(lags):
    ...
    ax[ii+1].text(6000, 0, f'avg corr: {cc}')
plt.tight_layout()
#plt.savefig(savef + 'Fig6_supp_deconvolution_v3.8.pdf')  

#%%
#lag = ['lag1', 'lag3'][1]
ff = '/media/nel/storage/fiola/R2_20190219/result/'
r = np.load(ff+f'Fig6_supp_dec_lag_v3.8.npy', allow_pickle=True).item()
#p = np.load(ff+f'Fig6c_prediction_{lag}_v3.8.npy', allow_pickle=True).item()
fig = plt.figure() 
ax1 = plt.subplot()
rr = list(r.values())
methods = list(r.keys())
...
plt.tight_layout()
plt.savefig(savef + 'Fig6_supp_deconvolution_b_v3.10.pdf')  
