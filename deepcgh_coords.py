...
class DeepCGH(object):
    ...
    def __init__(self,
        ...
        self.__check_avalability()
        try:
            self.masks = model_params['masks']
            self.num_neurons = len(self.masks)
    ...
    def __check_avalability(self):
        ...
        if not os.path.exists(self.absolute_file_path):
            ...
            print('Folder already exists.', self.absolute_file_path)
    ...
    def load_data(self, path, batch_size, epochs, shuffle):
        if isinstance(path, list) and ('tfrecords' in path[0]) and ('tfrecords' in path[1]):
            ...
            def val_func():
                return tf.data.TFRecordDataset(path[1],
                                      compression_type='GZIP',
                                      buffer_size=None,
                                      num_parallel_reads=2).map(__parse_image_function).batch(batch_size)
            def train_func():
                return tf.data.TFRecordDataset(path[0],
                                      compression_type='GZIP',
                                      buffer_size=None,
            ...
            def __make(example_proto):
                return example_proto, example_proto
            def rand_binary_gen():
                count = 0
                while count<self.N:
                    yield np.squeeze(np.array((np.random.rand(1, self.num_neurons)>0.5)*1, dtype=np.int32))
                    count += 1
            def eval_fn():
                return tf.data.Dataset.from_generator(rand_binary_gen,
                                                      output_types = tf.uint8,
                                                      output_shapes = tf.TensorShape([self.num_neurons])).map(__make).batch(batch_size)
            def train_fn():
                return eval_fn().repeat(epochs).prefetch(tf.data.experimental.AUTOTUNE)
            return train_fn, eval_fn
        
        
        else:
            raise NameError('You got a problem in your file name bruh')
    ...
    def __queued_predict_input_fn(self):
        ...
        dataset = tf.data.Dataset.from_generator(self.__generate_from_queue,
                                                 output_types = tf.uint8,
                                                 output_shapes = tf.TensorShape([1, self.num_neurons]))
        return dataset
    ...
    def __get_model_fn(self):
        ...
        masks = self.masks.reshape((self.num_neurons, -1)).astype(np.float32)
        
        def __img_synthesizer(x):
            x = tf.cast(x, tf.float32)
            # x = tf.expand_dims(tf.cast(x, tf.float32), axis=0)
            masks_sp = tf.sparse.from_dense(masks)
            return tf.reshape(tf.sparse.sparse_dense_matmul(x, masks_sp), (-1,)+self.shape)
        
        def __unet():
            n_kernels = self.n_kernels
            inp = Input(shape=[self.num_neurons], name='target')
            act_func = 'relu'
            # Synthesize Image
            x1_1 = Lambda(__img_synthesizer, name='Synth')(inp)
            # Interleave
            x1_1 = Lambda(interleave, name='Interleave')(x1_1)
            # Block 1
        ...
        def __ifft_AmPh(x):
            ...
            phase -= tf.reduce_min(phase, axis = [1, 2], keepdims = True)
            phase /= tf.reduce_max(phase, axis = [1, 2], keepdims = True)
            phase = phase*(2**8-1) 
            return phase
        ...
        def __big_loss(y_true, phi_slm):
            ...
            y_true = __img_synthesizer(y_true)
        ...
        def model_fn(features, labels, mode):
            ...
            phi_slm = unet(features, training = training)
        
            if mode == tf.estimator.ModeKeys.PREDICT:
                return tf.estimator.EstimatorSpec(mode, predictions = tf.cast(phi_slm, tf.uint8))
            
            else:
                acc = __big_loss(labels, phi_slm)
                ...
                if mode == tf.estimator.ModeKeys.EVAL:
                    ...
                    update_ops = unet.get_updates_for(None) + unet.get_updates_for(features)
