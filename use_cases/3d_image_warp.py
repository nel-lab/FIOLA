...
#         return interpolated
# #%% 2D
# shape = np.array([10,20])
# a = np.zeros(shape);
# a[shape[0]//2,shape[1]//2] = 1
# at = tf.convert_to_tensor(a[None,:,:,None])
# image = at
...
#%% 3D
shape = np.array([1001,1000,50])
a = np.zeros(shape, dtype=np.float32);
a[shape[0]//2,shape[1]//2,shape[2]//2] = 1
at = tf.convert_to_tensor(a[None,:,:,:,None], dtype=tf.float32)
if False:
    atc = tf.convert_to_tensor(a[None,:,:,:,None], dtype=tf.complex64)

image = at
flow = np.concatenate([2*np.ones((1,shape[0],shape[1],shape[2],1)),
                       -3*np.ones((1,shape[0],shape[1],shape[2],1)),
                       4*np.ones((1,shape[0],shape[1],shape[2],1))], axis=-1).astype(np.float32)
flow = tf.convert_to_tensor(flow, tf.float32)
at_t_3D= dense_image_warp_3D(at,flow)
...
# Build y_ref.
x0s, x1s = tf.meshgrid(
    tf.linspace(x_ref_min[0], x_ref_max[0], num=10),
    tf.linspace(x_ref_min[1], x_ref_max[1], num=10),
    indexing='ij')
...
flow = np.concatenate([2.24*np.ones((1,shape[0],shape[1],1)),-3.45*np.ones((1,shape[0],shape[1],1))], axis=-1)
at_t= dense_image_warp(at,flow)
at_t_ag= dense_image_warp_AG(at,flow)
#%%
plt.figure()
plt.imshow(tf.squeeze(at_t-at_t_ag))
plt.colorbar()
...
def dense_image_warp_3D(
    ...
    with tf.name_scope(name or "dense_image_warp_3D"):
