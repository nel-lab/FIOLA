...
"""
Illustration of the usage of FIOLA with calcium imaging data. USE THE demo_initialize_calcium.py FILE TO GENERATE THE HDF5 files necessary for 
initialize FIOLA.  
...
if estimate_baseline: # not thoroughly tested and computationally intensive for large files, it will estimate the baseline
    fio.fit_hals(mc_nn_mov, estimates.A, estimates.b)
    Ab = fio.Ab
else:
    Ab = np.hstack((estimates.A.toarray(), estimates.b))
    
fio = fio.create_pipeline(mc_nn_mov, np.vstack((estimates.C+estimates.YrA,estimates.f)), template, Ab, min_mov=min_mov)
#%%
