...
#    a = cm.load('/home/nellab/caiman_data/example_movies/n.01.01._rig__d1_512_d2_512_d3_1_order_F_frames_1825_.mmap', in_memory=True)
    template0 = np.median(a,axis=0)
    epsilon=0.00000001
    #import pdb; pdb.set_trace()
    shp = int(template0.shape[1]/4)
    template = template0[shp+10:-(10+shp),shp+10:-(shp+10), None, None]
...
#    import pdb; pdb.set_trace()
    ...
    AtA = Ab.T@Ab
    Atb = Ab.T@b
    n_AtA = np.array(np.linalg.norm(AtA, ord='fro')) #Frob. normalization
    print(n_AtA.dtype, type(n_AtA))
...
class Spikes(object):
    ...
    def __init__(self, model, y_old, x_old, mc0):
        self.model, self.y0, self.x0, self.mc0 = model, y_old, x_old, mc0
...
#        self.spike_input_q.enqueue([y, x, k, tht2])
    ...
    def extract(self):
        ...
        for elt in self.estimator.predict(input_fn=self.get_dataset, yield_single_examples=False):
            print(elt)
            self.output_q.put(elt.popitem())
...
#            self.output_q.enqueue(y)
    ...
    def get_dataset(self):
        dataset = tf.data.Dataset.from_generator(self.generator, 
                                                 ...
                                                 output_shapes=tf.TensorShape([1, 512, 512, 1]))
        return dataset
...
from classes2 import MotionCorrect, compute_theta2, NNLS
model, y_0, x_0, mc_0 = get_model()
#%%    
#from classes2 import MotionCorrect, NNLS, compute_theta2
spike_extractor = Spikes(model, y_0, x_0, mc_0)
print()
print()
print("out of init")
cfnn = []
