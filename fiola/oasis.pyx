...
cdef int fit_next_AR1(float y, float[:] s, float lg,
                      float[:] v, float[:] w, int[:] t, int[:] l, int i, int tt) nogil:
    v[i], w[i], t[i], l[i] = y, 1, tt, 1
    cdef float f
    if i > 0:
        f = exp(lg * l[i-1])
        s[tt] = y if (i==1 and v[0]<0) else y - v[i-1] / w[i-1] * f
    while (i > 0 and s[t[i]] < 0): # backtrack until violations fixed
        s[t[i]] = 0
        i -= 1
        # merge two pools
        v[i] += v[i+1] * f
        ...
        if i > 0:
            f = exp(lg * l[i-1])
            s[t[i]] = v[i] / w[i] if (i==1 and v[0]<0) else v[i] / w[i] - v[i-1] / w[i-1] * f
            
    i += 1
    return i

def par_fit_next_AR1(float[:] y, float[:,:] s, float[:] lg,
                     float[:,:] v, float[:,:] w, int[:,:] t, int[:,:] l, int[:] i, int tt):
    cdef int k, N
    N = y.shape[0]
    for k in prange(N, nogil=True):
        i[k] = fit_next_AR1(y[k],s[k],lg[k], v[k],w[k],t[k],l[k],i[k],tt)


cdef int fit_next_AR2(float[:] y, float[:] s, float d,
                      float[:] g11, float[:] g12, float[:] g11g11, float[:] g11g12,
                      float[:] v, float[:] w, int[:] t, int[:] l, int i, int tt) nogil:
...
def par_fit_next_AR2(float[:,:] y, float[:,:] s, float[:] d,
                     float[:,:] g11, float[:,:] g12, float[:,:] g11g11, float[:,:] g11g12,
                     float[:,:] v, float[:,:] w, int[:,:] t, int[:,:] l, int[:] i, int tt):
    cdef int k, N
    N = y.shape[0]
    for k in prange(N, nogil=True):
        i[k] = fit_next_AR2(y[k],s[k],d[k], g11[k],g12[k],g11g11[k],g11g12[k], v[k],w[k],t[k],l[k],i[k],tt)




def reconstruct_AR1(float[:,:] s, float[:,:] c, float[:] g,
                    float[:,:] v, float[:,:] w, int[:,:] t, int[:] i):
    cdef int n, k, N, T
    N = c.shape[0]
    T = c.shape[1]
...
def reconstruct_AR2(float[:,:] c, float[:] d, float[:,:] g,
                    float[:,:] v, float[:,:] w, int[:,:] t, int[:,:] l, int[:] i):
    cdef int n, k, j, N
    N = v.shape[0]
    for n in prange(N, nogil=True):
        c[n,0] = fmax(v[n,0], 0)
