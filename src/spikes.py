...
#        self.spike_input_q = mp.Queue()
        self.output_q = tf.queue.FIFOQueue(capacity=4, dtypes=tf.float32)
        self.zero_tensor = tf.convert_to_tensor([[0]], dtype=tf.float32)
        self.estimator = self.load_estimator()
        
        (y, x, k), tht2 = self.model([self.mc0, self.y0[None, :], self.x0[None, :], self.zero_tensor])
...
#        for i in range(1, idx+1):
        ...
        self.frame_input_q.enqueue(t[None, :])
        
        out = self.output_q.dequeue()
#        print(out.shape)
        return out
...
from classes import MotionCorrect, compute_theta2, NNLS
spike_extracter = Spikes()
cfnn = []
start = float(timeit.default_timer())
#cfnn.append(spike_extracter.get_spikes(50))
for i in range(1, 6):
    cfnn.append(tf.squeeze(spike_extracter.get_spikes(i)))
